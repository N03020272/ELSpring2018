import RPi.GPIO as GPIO from flask import Flask, render_template, 
request app = Flask(__name__) GPIO.setmode(GPIO.BCM)
...
for pin in pins:
   ...
   GPIO.output(pin, GPIO.LOW) @app.route("/") def main():
...
@app.route("/<changePin>/<action>") def action(changePin, action):
   ...
   return render_template('ledonoff.html', **templateData) if __name__ 
	== "__main__":
